...
async function scanGitRoot(gitRoot: string): Promise<CachedRepoScan | null> {
  ...
  if (nameStatus) {
    for (const statusLine of nameStatus.split('\n').filter(Boolean)) {
      ...
      files.push({ filePath, status, additions: stats.additions, deletions: stats.deletions, gitRoot, baseline: 'git' })
    }
  }
...
export async function getUnstagedChanges(
  ...
  // Session 快照不依赖 Git 扫描缓存；每次刷新都读取当前文件状态。
  ...
  // Git 结果优先；仅补充 Git/未追踪列表未覆盖的会话快照文件。
  const gitChangedPaths = new Set([
    ...files.map((file) => normalizeComparablePath(join(file.gitRoot, file.filePath))),
    ...untrackedFiles.map((file) => normalizeComparablePath(join(file.gitRoot, file.filePath))),
  ])
  for (const file of sessionFiles) {
    const absolutePath = normalizeComparablePath(join(file.gitRoot, file.filePath))
    if (!gitChangedPaths.has(absolutePath)) files.push(file)
  }
...
export async function getDiffContents(
  ...
  if (baseline === 'session') {
    ...
    return getAgentFileChangeTracker().getDiffContents(sessionId, gitRoot || dirPath, filePath)
  }

  const root = gitRoot || await findGitRoot(dirPath)

  // 无 git root：优先使用 Agent 首次写入前快照；没有旧快照时保留纯文件预览兼容行为。
  if (!root) {
    const comparisonRoot = gitRoot || dirPath
    const sessionDiff = sessionId
