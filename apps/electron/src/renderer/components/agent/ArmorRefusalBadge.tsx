 ...
 *
 * 位置：输入框上方常驻状态行（对应 gen-2 的 conversation.input.dock）。
 * 行为：
 * - 仅在 personalDirective 开启时显示；
 * - 会话流式进行中显示「思考中…」（绿色呼吸）；
 * - 最近一条助手消息命中拒绝话术 → 红色「✗ 拒绝: <短语>」；
...
import { armorScore, lastAssistantText } from '@/lib/armor-score'
...
export function ArmorRefusalBadge({ sessionId }: ArmorRefusalBadgeProps): React.ReactElement | null {
  ...
  const score = armorScore(lastAssistantText(messages))
  ...
  if (running) {
    ...
    const word = score.words[0] ?? '拒绝话术'
    label = `✗ 拒绝: ${word}`
    style = { ...PILL_BASE, ...STYLES.refusal }
    title = `检测到拒绝话术：${score.words.join('、') || word}（仅诊断显示，未修改模型行为）`
  } else if (score.verdict === 'fallback') {
    const word = score.words[0] ?? '改口话术'
    label = `⚠ 改口: ${word}`
    style = { ...PILL_BASE, ...STYLES.fallback }
    title = `检测到改口 / 安全替代话术：${score.words.join('、') || word}`
  } else {
    label = '✓ 破甲生效'
    style = { ...PILL_BASE, ...STYLES.pass }
    title = '破甲已开启 · 最近回复未命中拒绝 / 改口话术'
  }
